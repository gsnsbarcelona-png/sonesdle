/**
 * ParticlesComponent — Single Responsibility: particulas de fondo
 * y confetti de victoria. No conoce nada del juego.
 */
export class ParticlesComponent {
  static #CONFETTI_COLORS = [
    '#c89b3c', '#f0e6d3', '#0bc4c4', '#00ff88', '#ffffff', '#ff8c55',
  ];
  static #DEFEAT_COLORS = [
    '#ff2222', '#880000', '#444444', '#222222', '#661111', '#992222',
  ];

  #pCanvas;
  #pCtx;
  #cCanvas;
  #cCtx;
  #particles   = [];
  #confetti    = [];
  #cAnimId      = null;
  #resizeTimer  = null;
  #rafParticles;
  #rafConfetti;

  /**
   * @param {{
   *   particleCanvas: HTMLCanvasElement,
   *   confettiCanvas: HTMLCanvasElement
   * }}
   */
  constructor({ particleCanvas, confettiCanvas }) {
    this.#pCanvas = particleCanvas;
    this.#pCtx    = particleCanvas.getContext('2d');
    this.#cCanvas = confettiCanvas;
    this.#cCtx    = confettiCanvas.getContext('2d');

    // Callbacks estables: se crean una vez, no en cada frame
    this.#rafParticles = () => this.#animParticles();
    this.#rafConfetti  = () => this.#animConfetti();

    window.addEventListener('resize', () => {
      clearTimeout(this.#resizeTimer);
      this.#resizeTimer = setTimeout(() => {
        this.#resize();
        this.#initParticles();
      }, 120);
    });
    this.#resize();
    this.#initParticles();
    this.#animParticles();
  }


  launchConfetti() {
    const w = this.#cCanvas.width;
    const h = this.#cCanvas.height;
    const colors = ParticlesComponent.#CONFETTI_COLORS;

    this.#confetti = Array.from({ length: 140 }, () => ({
      x:     Math.random() * w,
      y:     Math.random() * -h * 0.5,
      w:     Math.random() * 10 + 5,
      h:     Math.random() * 6 + 3,
      color: colors[Math.floor(Math.random() * colors.length)],
      vx:    (Math.random() - 0.5) * 3.5,
      vy:    Math.random() * 3 + 2,
      rot:   Math.random() * Math.PI * 2,
      rotV:  (Math.random() - 0.5) * 0.18,
      alpha: 1,
    }));

    if (this.#cAnimId) cancelAnimationFrame(this.#cAnimId);
    this.#animConfetti();
  }

  launchDefeat() {
    const w = this.#cCanvas.width;
    const h = this.#cCanvas.height;
    const colors = ParticlesComponent.#DEFEAT_COLORS;

    // Gotas/cenizas que caen desde arriba, lentas y pesadas
    this.#confetti = Array.from({ length: 80 }, () => ({
      x:     Math.random() * w,
      y:     Math.random() * -h * 0.3,
      w:     Math.random() * 6 + 2,
      h:     Math.random() * 6 + 2,
      color: colors[Math.floor(Math.random() * colors.length)],
      vx:    (Math.random() - 0.5) * 1.2,
      vy:    Math.random() * 2 + 1,
      rot:   Math.random() * Math.PI * 2,
      rotV:  (Math.random() - 0.5) * 0.06,
      alpha: 0.85,
    }));

    if (this.#cAnimId) cancelAnimationFrame(this.#cAnimId);
    this.#animConfetti();
  }

  stopConfetti() {
    if (this.#cAnimId) {
      cancelAnimationFrame(this.#cAnimId);
      this.#cAnimId = null;
    }
    this.#cCtx.clearRect(0, 0, this.#cCanvas.width, this.#cCanvas.height);
    this.#confetti = [];
  }


  #resize() {
    this.#pCanvas.width  = this.#cCanvas.width  = window.innerWidth;
    this.#pCanvas.height = this.#cCanvas.height = window.innerHeight;
  }

  #initParticles() {
    const n = Math.floor((window.innerWidth * window.innerHeight) / 14000);
    this.#particles = Array.from({ length: n }, () => ({
      x:  Math.random() * this.#pCanvas.width,
      y:  Math.random() * this.#pCanvas.height,
      r:  Math.random() * 1.4 + 0.4,
      vx: (Math.random() - 0.5) * 0.28,
      vy: (Math.random() - 0.5) * 0.28,
      a:  Math.random() * 0.4 + 0.08,
    }));
  }

  #animParticles() {
    const ctx = this.#pCtx;
    const w   = this.#pCanvas.width;
    const h   = this.#pCanvas.height;
    ctx.clearRect(0, 0, w, h);

    for (const p of this.#particles) {
      p.x += p.vx; p.y += p.vy;
      if (p.x < 0) p.x = w; if (p.x > w) p.x = 0;
      if (p.y < 0) p.y = h; if (p.y > h) p.y = 0;
      ctx.beginPath();
      ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(200,155,60,${p.a})`;
      ctx.fill();
    }

    requestAnimationFrame(this.#rafParticles);
  }

  #animConfetti() {
    const ctx = this.#cCtx;
    const w   = this.#cCanvas.width;
    const h   = this.#cCanvas.height;
    ctx.clearRect(0, 0, w, h);

    let alive = false;
    for (const p of this.#confetti) {
      p.x += p.vx; p.y += p.vy;
      p.rot += p.rotV;
      if (p.y > h * 0.65) p.alpha -= 0.018;
      if (p.alpha > 0) alive = true;

      ctx.save();
      ctx.globalAlpha = Math.max(0, p.alpha);
      ctx.translate(p.x, p.y);
      ctx.rotate(p.rot);
      ctx.fillStyle = p.color;
      ctx.fillRect(-p.w / 2, -p.h / 2, p.w, p.h);
      ctx.restore();
    }

    if (alive) this.#cAnimId = requestAnimationFrame(this.#rafConfetti);
  }
}
